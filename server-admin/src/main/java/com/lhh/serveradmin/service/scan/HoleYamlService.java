...
//                String newPath = Const.STR_SLASH + String.join(Const.STR_SLASH, newFolderList);
    ...
    public R uploadFiles(List<MultipartFile> files, List<String> paths, Integer toolType, Long folderId) {
        ...
        List<HoleYamlFolderEntity> folderList = holeYamlFolderFeign.list(new HashMap<String, Object>() {{
            put("findId", folderId);
        }});
        // 确保生成了第一个目录
        if (CollectionUtils.isEmpty(folderList)) {
            if (CollectionUtils.isEmpty(holeYamlFolderFeign.list(new HashMap<String, Object>() {{
                put("id", folderId);
            }}))) {
    ...
    public R delete(List<Long> ids) {
        ...
        List<HoleYamlEntity> list = holeYamlFeign.list(new HashMap<String, Object>() {{
            put("idList", ids);
        }});
