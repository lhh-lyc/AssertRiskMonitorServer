package com.lhh.serveradmin.jwt.utils;
...
public class PassJavaJwtTokenUtil {
    ...
    public void delCacheToken(String userId) {
        stringRedisTemplate.opsForHash().delete(JWT_CACHE_KEY + userId, ACCESS_TOKEN);
        stringRedisTemplate.opsForHash().delete(JWT_CACHE_KEY + userId, REFRESH_TOKEN);
    }
    ...
    public String getUserId() {
        String userId;
        try {
            Claims claims = getClaimsFromToken(getToken());
            userId = claims.getSubject();
        } catch (Exception e) {
            userId = null;
        }
