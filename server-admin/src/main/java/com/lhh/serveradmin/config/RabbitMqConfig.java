...
public class RabbitMqConfig {
    ...
    @Value("${mqtt-setting.project-pub-topic}")
    private String projectRubTopic;
    @Value("${mqtt-setting.reDomain-route-key}")
    private String reDomainRouteKey;
    @Value("${mqtt-setting.reDomain-pub-topic}")
    private String reDomainPubTopic;
    ...
    @Bean
    public Queue queueA() {
        return new Queue(projectRubTopic, true); //队列持久：不会随着服务器重启造成丢失
    }
    ...
    public Binding binding() {
        //绑定队列到交换机上通过路由
        return BindingBuilder.bind(queueA()).to(defaultExchange()).with(projectRouteKey);
    }

    //声明队列
    @Bean
    public Queue queueB() {
        return new Queue(reDomainPubTopic, true); //队列持久：不会随着服务器重启造成丢失
    }
    ...
    public Binding bindingB() {
        //绑定队列到交换机上通过路由
        return BindingBuilder.bind(queueB()).to(defaultExchange()).with(reDomainRouteKey);
    }

    //声明队列
    @Bean
    public Queue queueC() {
        return new Queue(reIpPubTopic, true); //队列持久：不会随着服务器重启造成丢失
    }
